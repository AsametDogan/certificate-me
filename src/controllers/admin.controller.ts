...
import { IAssignment, ICertificate, IRequestWithUser } from "../interfaces";
...
const sendCertifiactes = async (req: IRequestWithUser, res: Response) => {
    ...
    try {
        const badge = await CertificateModel.findById(badgeId)
        if (!badge) {
...
const createCertifiacte = async (req: IRequestWithUser, res: Response) => {

    try {
        const { title, categoryId, totalCount, price, attainerRoles } = req.body;
        const newBadge = new CertificateModel({
            title,
...
const deleteCertificate = async (req: IRequestWithUser, res: Response) => {

    try {
        const { badgeId } = req.body
        const deletedBadge: ICertificate | null = await CertificateModel.findByIdAndDelete(badgeId);
        if (!deletedBadge) {
...
const updateBadge = async (req: IRequestWithUser, res: Response) => {
    ...
    try {
        ...
        const updatedBadge = await CertificateModel.findByIdAndUpdate(
            badgeId,
            {
                $set: updateFields,
            },
...
/* const getAllBadgesAdmin = async (req: Request, res: Response) => {
    ...
    try {
        const badges = await CertificateModel.find().populate({
            path: "categoryId",
            model: "Category"
        });
...
export {
    sendCertifiactes as sendBadges,
    createCertifiacte as createBadge,
    setBadgeAttainer,
    deleteCertificate as deleteBadge,
